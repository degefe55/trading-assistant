...
def main():
    ...
    try:
        ...
        if brief_type in ("premarket", "test"):
            ...
            # GitHub Actions / manual env-driven runs are by definition manual
            run_midsession_check(manual_trigger=True)
...
def run_premarket_brief():
    ...
    analyst.reset_cancel("premarket")
    ...
    try:
        ...
        cancelled = True
        log_event("WARN", "main",
                  f"Pre-market brief cancelled mid-run after analyzing "
                  f"{len(position_analyses)} positions, "
                  f"{len(focus_analyses)} focus, "
                  f"{len(watchlist_analyses)} watchlist tickers")

    macro = data_router.get_macro()
    summary = get_run_summary()
    run_cost = summary["total_cost_usd"]

    if cancelled and not (position_analyses or focus_analyses or watchlist_analyses):
        # Nothing meaningful collected before cancel — send a short note instead
        telegram_client.send_message(
            "🛑 <b>Pre-market brief cancelled</b>\n"
            f"No analyses had completed yet.\n"
            f"💰 Run cost so far: <code>${run_cost:.4f}</code>"
        )
        log_event("INFO", "main", "Pre-market brief: cancelled before any output")
        return
    ...
    try:
        ...
        log_event("WARN", "main", f"MessageMap write failed: {e}")

    log_event("INFO", "main",
              "Pre-market brief sent" + (" (partial — cancelled)" if cancelled else ""))


def run_midsession_check(manual_trigger: bool = False):
    """Mid-session: only alert on material changes when scheduled.

    When `manual_trigger=True` (e.g. /run midsession from Telegram, or
    GitHub Actions workflow_dispatch), always send a Telegram message —
    even if nothing material was found — so the user gets confirmation
    that the run actually executed.

    When `manual_trigger=False` (scheduled run), stay silent unless
    something is material. This is the cost-saving / attention-saving
    design from Phase A.

    Either way, log a one-line summary so the Logs tab shows midsession
    actually ran ("checked N positions, M material") even when silent.
    """
    if "US" not in ACTIVE_MARKETS:
        return

    analyst.reset_cancel("midsession")
    ...
    try:
        ...
        cancelled = True
        log_event("WARN", "main",
                  f"Mid-session check cancelled mid-run "
                  f"({len(material_changes)} alerts collected)")

    # Always log a summary so the Logs tab shows the run happened
    log_event("INFO", "main",
              f"Mid-session: checked {checked} positions, "
    ...
    if cancelled:
        if material_changes:
            ...
            telegram_client.send_message("🛑 <b>Mid-session check cancelled</b>")
        return

    # Branch: material changes found — always send (whether scheduled or manual)
    if material_changes:
        message = brief_composer.compose_midsession_check(
            material_changes, summary["total_cost_usd"])
        if message:
            ...
            try:
                rec_ids = [a.get("rec_id") for a in material_changes
                           if a.get("rec_id")]
                if msg_id and rec_ids:
                    sheets.record_message_recids(msg_id, "midsession", rec_ids)
            except Exception as e:
                log_event("WARN", "main", f"MessageMap write failed: {e}")
            log_event("INFO", "main", "Mid-session alert sent")
        return

    # Branch: nothing material
    if manual_trigger:
        # User asked for it — confirm the run actually happened
        now_ksa = datetime.now(KSA_TZ)
        confirm = (
            f"⚪ <b>Mid-session check</b>  <i>{now_ksa.strftime('%H:%M KSA')}</i>\n"
            f"─────────────\n"
        ...
        log_event("INFO", "main", "Mid-session manual: sent 'nothing material' confirmation")
    else:
        # Scheduled run, nothing material — silent by design
        log_event("INFO", "main", "Mid-session scheduled: silent (nothing material)")
...
def run_preclose_verdict():
    ...
    try:
        ...
        cancelled = True
        log_event("WARN", "main",
                  f"Pre-close verdict cancelled mid-run "
                  f"({len(analyses)} of {len(us_positions)} positions analyzed)")

    summary = get_run_summary()
    if cancelled and not analyses:
        telegram_client.send_message(
            "🛑 <b>Pre-close verdict cancelled</b>\n"
    ...
    try:
        ...
        if msg_id and rec_ids:
            sheets.record_message_recids(msg_id, "preclose", rec_ids)
    except Exception as e:
        log_event("WARN", "main", f"MessageMap write failed: {e}")

    log_event("INFO", "main",
              "Pre-close verdict sent" + (" (partial — cancelled)" if cancelled else ""))
