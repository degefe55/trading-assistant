...
- Persists every successful recommendation to the Recommendations tab
  (unblocks Phase D outcome grading).
- Honors a brief-level cancellation flag (used by /cancel command) by
  raising BriefCancelled before the next ticker's Sonnet call. The
  in-flight call cannot be aborted; the cancel only stops further calls.
"""
import json
import re
from datetime import datetime
from zoneinfo import ZoneInfo
from config import USD_TO_SAR, DEFAULT_RULES, KSA_TZ
from core import data_router, technical, halal, claude_client, sheets
from core.logger import log_event


class BriefCancelled(Exception):
    """Raised inside analyze_ticker when a /cancel was requested.

    Brief functions in main.py catch this and stop their loop early.
    """
    pass
...
def request_cancel(brief_type: str) -> bool:
    """Set the cancel flag for a brief. Returns True if it was previously False.

    Called by /cancel command. The currently-executing analyze_ticker()
    raises BriefCancelled before its next Claude call.
    """
    if brief_type not in _cancel_flags:
        return False
    was_set = _cancel_flags[brief_type]
    _cancel_flags[brief_type] = True
    return not was_set


def reset_cancel(brief_type: str):
    """Clear the cancel flag. Called by main.py at the start of each brief."""
    if brief_type in _cancel_flags:
        _cancel_flags[brief_type] = False


def _check_cancelled(context_type: str):
    """Raise BriefCancelled if the matching brief's flag is set."""
    if context_type in _cancel_flags and _cancel_flags[context_type]:
        log_event("WARN", "analyst",
                  f"Cancellation honored for {context_type}")
        raise BriefCancelled(context_type)


def _market_status_note() -> str:
    """Returns a context note for Claude about market session state."""
    now = datetime.now(KSA_TZ)
    h, m = now.hour, now.minute
    total = h * 60 + m
    # US market in KSA: 16:30 - 23:00 (standard) or 17:30-00:00 (DST shifts)
    if total < 16 * 60 + 30:
        return ("MARKET STATUS: PRE-MARKET (US not open yet). "
                "Low volume is NORMAL — do NOT flag thin volume as suspicious. "
                "Prices may lag the eventual open.")
    if total > 23 * 60:
        return ("MARKET STATUS: AFTER-HOURS (US closed). "
                "Low volume is NORMAL — do NOT flag thin volume as suspicious.")
    return "MARKET STATUS: OPEN — volume signals are meaningful."
...
def analyze_ticker(ticker: str, market: str = "US",
    ...
    Honors brief-level cancellation: raises BriefCancelled if the matching
    brief's flag was set via request_cancel() before this call started.
    """
    ...
    # Persist this recommendation so Phase D outcome grading can later
    # compare actual price moves against the bot's call. Failures here
    # are logged but never propagate — losing a log row is acceptable;
    # losing the brief is not.
    try:
        rec_id = sheets.append_recommendation({
            "brief_type": context_type,
            "ticker": ticker,
            "market": market,
